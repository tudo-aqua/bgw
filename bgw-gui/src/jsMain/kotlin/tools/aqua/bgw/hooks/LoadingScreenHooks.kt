...
/**
 * Custom React hook for pre-loading images with progress tracking.
 *
 * This hook handles:
 * - Loading images from a list of paths
 * - Tracking loading progress
 * - Updating progress UI elements
 * - Coordinating with logo animation timing
 * - Automatically hiding the loading screen when complete
 *
 * @param loadingScreenData The loading screen configuration and state
 */
internal fun useImagePreloading(loadingScreenData: LoadingScreenData) {
  useEffect(loadingScreenData.visible, loadingScreenData.imagePaths) {
    if (!loadingScreenData.visible || loadingScreenData.totalImages == 0) {
      return@useEffect
    }

    console.log("Starting image pre-loading for ${loadingScreenData.totalImages} images")

    val startTime = window.performance.now()
    var loadedCount = 0
    val logoAnimationDuration = 1500.0 // SVG animation is 1500ms
    var logoAnimationFinished = false
    var allImagesLoaded = false

    fun checkAndHideLoadingScreen() {
      return
      val currentTime = window.performance.now()
      val elapsedTime = currentTime - startTime
      val minimumTimeElapsed = elapsedTime >= (logoAnimationDuration + loadingScreenData.minimumDisplayTime)

      if (allImagesLoaded && logoAnimationFinished && minimumTimeElapsed) {
        console.log("All conditions met - hiding loading screen (${loadedCount} images loaded)")

        // Send event to backend to hide loading screen
        val event = js("new CustomEvent('BGW_LOADING_COMPLETE', { detail: 'complete' })")
        document.dispatchEvent(event)

        // Hide loading screen with fade out
        val loadingScreen = document.getElementById("bgw_loading_screen")
        if (loadingScreen != null) {
          loadingScreen.asDynamic().style.opacity = "0"
          loadingScreen.asDynamic().style.transition = "opacity 0.3s ease-out"
          window.setTimeout({
            loadingScreen.asDynamic().style.display = "none"
          }, 300)
        }
      } else if (allImagesLoaded && logoAnimationFinished && !minimumTimeElapsed) {
        // Wait for minimum time
        val remainingTime = (logoAnimationDuration + loadingScreenData.minimumDisplayTime - elapsedTime).toInt()
        console.log("Waiting ${remainingTime}ms for minimum display time")
        window.setTimeout({
    ...
    // Track when logo animation finishes
    val logoTimeoutId = window.setTimeout({
      console.log("Logo animation finished")
      logoAnimationFinished = true
      checkAndHideLoadingScreen()
    }, logoAnimationDuration.toInt())

    fun updateProgressUI(current: Int, total: Int) {
      // Update progress text
      val progressText = document.getElementById("bgw_loading_progress_text")
      if (progressText != null) {
        progressText.textContent = "Loading assets: $current / $total"
      }

      // Update progress bar width
      val progressBar = document.getElementById("bgw_loading_progress_bar")
      if (progressBar != null) {
        val progress = (current.toDouble() / total.toDouble()) * 100.0
        progressBar.asDynamic().style.width = "${progress}%"
      }
    }

    // Pre-load each image
    loadingScreenData.imagePaths.forEach { path ->
      // Use js() to properly construct Image with 'new' operator
      val img = js("new Image()") as Image

      img.onload = {
        loadedCount++
        console.log("Loaded image $loadedCount/${loadingScreenData.totalImages}: $path")

        updateProgressUI(loadedCount, loadingScreenData.totalImages)

        // Check if all images are loaded
        if (loadedCount >= loadingScreenData.totalImages) {
