...
open class GridPane<T : ComponentView>(
   ...
   */
  operator fun get(columnIndex: Int, rowIndex: Int): T? = grid[columnIndex, rowIndex]

  /**
   * Sets content of desired grid cell. Overrides existing component in this cell. Pass `null` to
   * remove a component.
   *
   * @param columnIndex Column index in grid.
   * @param rowIndex Row index in grid.
  ...
  internal fun layout() {
    grid.forEach { cell ->
      ...
      component?.let {
        ...
        if (pos != null) {
          it.inParentPosXProperty.setSilent(pos.xCoord)
          it.inParentPosYProperty.setSilent(pos.yCoord)
        }
      }
    }
  }
  ...
  /**
   * Sets centering mode of desired grid cell with given [Alignment]. Overrides existing mode in
   * this cell.
   *
   * @param columnIndex Column index in grid.
   * @param rowIndex Row index in grid.
  ...
  /**
   * Sets centering mode of desired column in grid with given [Alignment]. Overrides existing mode
   * in the whole column.
   *
   * @param columnIndex Column index in grid.
  ...
  /**
   * Sets centering mode of desired row in grid with given [Alignment]. Overrides existing mode in
   * the whole row.
   *
   * @param rowIndex Row index in grid.
  ...
  /**
   * Sets centering mode of all cells in the grid with given [Alignment]. Overrides existing modes
   * of all cells.
   *
  ...
  /**
   * Manually set column width of one column. Overrides automatic resizing based on content from
   * this column.
   *
   * @param columnIndex Target column.
  ...
  /**
   * Manually set row height of one row. Overrides automatic resizing based on content from this
   * row.
   *
   * @param rowIndex Target row.
  ...
  /**
   * Removes all empty outer rows and columns e.g. all rows and columns counted from left, right,
   * top and bottom that have no views in their cells.
   *
   * That means after calling the first and last row as well as the first and last column have at
   * least one component in their cells.
   *
  ...
  /**
   * Adds the desired amount of columns between column <b>columnIndex - 1</b> and
   * <b>columnIndex</b>. New columns get `null`-initialized.
   *
  ...
  /**
   * Removes the desired column in the grid. Removes all, views in the column. If there is no column
  ...
  /**
   * Adds the desired amount of rows between row <b>rowIndex</b> and <b>rowIndex + 1</b>. New rows
   * get `null`-initialized.
   *
  ...
  /**
   * Function returning a contained child's coordinates within this [GridPane] relative to the top
   * left corner.
   *
  ...
  override fun getChildPosition(child: ComponentView): Coordinate? =
      grid.filter { it.component == child }.map { getRelativeChildOffset(it) }.firstOrNull()
  ...
  private fun getRelativeChildOffset(it: GridIteratorElement<T>): Coordinate {
    if (it.component == null) return Coordinate(0.0, 0.0)
    val cols = grid.getColumnWidths().toMutableList().subList(0, it.columnIndex)
    val rows = grid.getRowHeights().toMutableList().subList(0, it.rowIndex)
   ...
   *
   * Iteration is columns-first which means that the iterator starts at cell [0,0] and then proceeds
   * iterating through the first row from left to right. When reaching the end of a row it proceeds
   * to the next one resetting the column pointer to 0.
   *
   * [Iterator.hasNext] Returns `true` if at least one cell is left to iterate.
   *
   * [Iterator.next] Returns a data object [GridIteratorElement] that contains the current row and
   * column as well as the component itself. Refer to the [GridIteratorElement] documentation for
   * further information.
   *
