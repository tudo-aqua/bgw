...
internal class HexGridGameScene : BoardGameScene() {
    private val hexGrid = HexagonGrid<HexagonView>(
        ...
        onMouseClicked = {
            placeOnHexGrid(HexagonView(visual = ColorVisual(Color(0, 0, 255)), size = 40).apply {})
        }
    }
    ...
    private val cameraPane = CameraPane(
        width = 1200, height = 800, target = Pane<ComponentView>(
            width = 1920, height = 1080, posX = 0, posY = 0, visual = ImageVisual("assets/3.jpg")
        ), posX = 200, posY = 100, visual = ColorVisual.YELLOW, limitBounds = true
    ).apply {
        interactive = true
    ...
    val centerDot = Label(posX = cameraPane.width / 2 - 2 + cameraPane.posX,
    ...
    val panButton = Button(
        posX = 5, posY = 1020, width = 100, height = 50, text = "Pan By", visual = ColorVisual(Color(0, 255, 0))
    ).apply { onMouseClicked = { cameraPane.panBy(200, 100) } }

    val panZeroButton = Button(
        posX = 105, posY = 1020, width = 100, height = 50, text = "Pan", visual = ColorVisual(Color(0, 125, 0))
    ).apply { onMouseClicked = { cameraPane.pan(0, 0, smooth = false) } }

    val zoomButton = Button(
        posX = 205, posY = 1020, width = 100, height = 50, text = "Zoom", visual = ColorVisual(Color(0, 255, 0))
    ).apply {
        onMouseClicked = {
    ...
    private val hexPointy = HexagonView(
        posX = 800,
        posY = 200,
    ...
    private val hexFlat = HexagonView(
        posX = 900, posY = 200, visual = ColorVisual(Color(0, 255, 0)), size = 50, orientation = HexOrientation.FLAT_TOP
    )

    fun placeOnHexGrid(hexagon: HexagonView) {
        hexGrid[10, 0] = HexagonView(
            visual = ColorVisual(Color(255, 0, 0)), size = 50, orientation = HexOrientation.POINTY_TOP
        ).apply {
            onMouseClicked = {
        ...
        for (q in -1..1) {
            for (r in -1..1) {
                if (q + r >= -1 && q + r <= 1) {
                    ...
                    val hexagon = HexagonView(visual = ColorVisual.LIGHT_GRAY, size = 40).apply {
                        onMouseClicked = {
