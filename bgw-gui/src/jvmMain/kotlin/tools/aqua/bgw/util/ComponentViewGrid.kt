...
internal data class ComponentViewGrid<T : ComponentView>(var rows: Int, var columns: Int) :
  ...
  */
  ...
  var columns = columns
    ...
    internal fun clone(): ComponentViewGrid<T> {
        return this.copy().apply target@{
            this@target.grid = this@ComponentViewGrid.grid.map { it.copyOf() }.toTypedArray()
            this@target.centeringModes = this@ComponentViewGrid.centeringModes.map { it.copyOf() }.toTypedArray()
            this@target.rowHeights = this@ComponentViewGrid.rowHeights.copyOf()
            this@target.columnWidths = this@ComponentViewGrid.columnWidths.copyOf()
        }
    }
}
