...
// region Util
/**
 * MenuSceneBuilder is a static class, that can be used to build [MenuScene]s. It is meant to be
 * used in conjunction with the internal domain specific language in [tools.aqua.bgw.components.dsl]
 * .
 *
 ...
 */
fun <T> MenuScene.grid(rows: Int, cols: Int, func: (GridPane<T>.() -> Unit)): GridPane<T> where
T : StaticComponentView<T> {
...
// region ToggleGroup
/**
 * Creates a new [ToggleGroup] and sets it for all [ToggleButton]s that get created in the context
 * of the receiver [ToggleGroupBuilder] of [func]. Also adds all those [ToggleButton]s to the
 * receiver [MenuScene].
 *
...
/**
 * [ToggleGroupBuilder] may be used to build [ToggleGroup]s in a semantically more appealing way,
 * than to set the [ToggleGroup] on every [ToggleButton].
 */
...
/**
 * Creates a new [ToggleButton], applies the [func] and adds it to the context of the receiver
 * [ToggleGroupBuilder].
 *
...
/**
 * Creates a new [RadioButton], applies the [func] and adds it to the context of the receiver
 * [ToggleGroupBuilder].
 *
