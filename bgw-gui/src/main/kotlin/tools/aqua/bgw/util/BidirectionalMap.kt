...
open class BidirectionalMap<T : Any, R : Any>(vararg elements: Pair<T, R>) {
    ...
    operator fun get(it: T): Any {
        if (containsForward(it)) return forward(it)
     ...
     * @param it Key (from the domain) to set the value for.
     * @param value Value (from the co-domain) to set for given domain key.
     * @return Pair of overwritten pairs. First element is the pair that was overwritten in the domain (or null), second element is the pair that was overwritten in the co-domain (or null).
     */
    ...
    fun putAll(bidirectionalMap: BidirectionalMap<T, R>) {
        bidirectionalMap.entries.forEach { entry ->
