...
class TetrisGameScene : BoardGameScene(background = ColorVisual.WHITE) {
  ...
  val pointsLabel: Label = Label(posX = 1250, posY = 40, text = "0 Points", font = Font(size = 20))

  /** Label displaying current speed. */
  val speedLabel: Label = Label(posX = 1250, posY = 100, width = 200, font = Font(size = 20))

  /** Preview 1. */
  val preview1: TetrisPreview =
      TetrisPreview(posX = 40 + 1.5 * cellSize, posY = 1040 - 2 * cellSize, cellSize = cellSize)

  /** Preview 2. */
  val preview2: TetrisPreview =
      TetrisPreview(posX = 80 + 4.5 * cellSize, posY = 1040 - 2 * cellSize, cellSize = cellSize)

  /** Preview 3. */
  val preview3: TetrisPreview =
      TetrisPreview(posX = 120 + 7.5 * cellSize, posY = 1040 - 2 * cellSize, cellSize = cellSize)

  /** Tetris grid. */
  val tetrisGrid: GridPane<Label> =
      GridPane<Label>(
              posX = 1920 / 2,
              posY = 1080 / 2,
              columns = 12,
