...
class UIComponentExample(roomId: String) :
    BoardGameApplication("UIComponent Example", width = 800, height = 1000, roomId = roomId) {
  private val gameScene1 =
      BoardGameScene(width = 800).apply {
        this.opacity = 1.0
        this.background = ColorVisual.LIGHT_GRAY
      }
  private val gameScene2 =
      BoardGameScene(width = 800).apply {
        this.opacity = 1.0
  ...
  init {
    ...
    button.onMouseClicked = { showGameScene(gameScene2) }

    val button2 =
        Button(
            posX = 450, posY = 100, text = "I am a second Button.", visual = ColorVisual.DARK_GRAY)

    button2.onMouseClicked = { showGameScene(gameScene1) }
