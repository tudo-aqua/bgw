...
class ContainerExample : BoardGameApplication("Container Example") {
  ...
  private val cardStack: CardStack<CardView> =
      CardStack<CardView>(
          ...
          .apply {
            addAll(
                generateCards(0, 5).map {
                  it.apply {
                    showBack()
                    isDraggable = true
                    onDragGestureStarted = { showFront() }
                  }
                })
            dropAcceptor = { it.draggedComponent is CardView }
            onDragDropped = { add((it.draggedComponent as CardView).apply { showBack() }) }
          }

  private val area: Area<TokenView> =
      Area(
  ...
  private val linearLayout: LinearLayout<CardView> =
      LinearLayout<CardView>(
          ...
          .apply {
            ...
            dropAcceptor = { it.draggedComponent is CardView }
            onDragDropped = { add((it.draggedComponent as CardView).apply { showFront() }) }
          }

  private val satchel: Satchel<TokenView> =
      Satchel(
          height = 200, width = 200, posX = 1010, posY = 590, visual = ColorVisual(0, 255, 0, 80))

  private val alignmentControls: Pane<UIComponent> =
      Pane<UIComponent>(posX = 50, posY = 335, width = 0, height = 0).apply {
        ...
        add(
            Button(width = 130, posX = 220, posY = 120, text = "VERTICAL").apply {
              visual = ColorVisual.LIGHT_GRAY
              onMouseClicked = { linearLayout.orientation = Orientation.VERTICAL }
            })
        add(
            Button(width = 130, posX = 370, posY = 120, text = "HORIZONTAL").apply {
              visual = ColorVisual.LIGHT_GRAY
              onMouseClicked = { linearLayout.orientation = Orientation.HORIZONTAL }
            })
      }
  ...
  private fun generateCards(row: Int, numCards: Int): Iterable<CardView> {
    ...
    return (0 until numCards)
        ...
        .map { imageVisual -> CardView(front = imageVisual, back = cardBackImageVisual) }
  }
}
