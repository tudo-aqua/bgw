    ...
    separatedTokens.forEach((tokens) => {
      ...
      for (const propertyName in distinctAssignments) {
        ...
        if (constructorProperty && constructorProperty.primitive) {
          ...
          object[propertyName] = this.getCompoundVisual(
            this.getKotlinTokens(distinctAssignments[propertyName]),
            true
          );
      ...
      for (const propertyName in distinctApplys) {
        ...
        if (constructorProperty && constructorProperty.primitive) {
          ...
          if (object[propertyName] && object[propertyName].propType) {
            let typedValue = this.getTypedValueApply(
              distinctApplys[propertyName],
  ...
  getCompoundVisual(tokens: Token[], topLayer: boolean = true) {
    ...
    if (filteredTokens[0].value == "Visual") {
      ...
      const compoundAssignments =
        this.getConstructorArguments(filteredTokens).constructor;

      const compoundVisuals: SingleLayerVisualData[] = compoundAssignments.map(
        (assignment) => {
          return this.getCompoundVisual(
            this.getKotlinTokens(assignment.value),
            false
          );
        }
      );
      ...
      if (colorVisualArguments.length == 1) {
        colorVisual.color = this.getColor(
          this.getKotlinTokens(colorVisualArguments[0].value)
        );
