...
import {colourNameToHex, constructors, enums, specialConstructors} from "@/lib/constructors.ts";
import {isNumber} from "@/lib/utils.ts";
import {ColorVisualData, CompoundVisualData, SingleLayerVisualData} from "@/lib/components.ts";
...
function Parser({ onCodeChange } : ParserProps) {
    ...
    const tryParse = (value: string) => {
        ...
        separatedTokens.forEach((tokens) => {
            ...
            for (const propertyName in distinctAssignments){
                ...
                let typedValue = getTypedValue(distinctAssignments[propertyName], constructorProperty)
                ...
                if (constructorProperty && constructorProperty.primitive){
                    ...
                    object[propertyName] = getCompoundVisual(getKotlinTokens(distinctAssignments[propertyName]))
                } else if(constructorProperty && constructorProperty.type == "Font"){
                    object[propertyName] = getFont(getKotlinTokens(distinctAssignments[propertyName]))
            ...
            for (const propertyName in distinctApplys){
                let constructorProperty = constructors[component.value].find((constructor) => constructor.property == propertyName)
                let typedValue = getTypedValue(distinctApplys[propertyName], constructorProperty)
                ...
                if (constructorProperty && constructorProperty.primitive){
                    ...
                    if(object[propertyName] && object[propertyName].propType) {
                        let typedValue = getTypedValueApply(distinctApplys[propertyName], object[propertyName].propType)
                        object[propertyName].value = typedValue
                    }
                }
            }
    ...
    function getCompoundVisual(tokens: Token[], topLayer : boolean = true) {
        ...
        if(filteredTokens[0].value == "Visual") {
            ...
            const compoundAssignments = getConstructorArguments(filteredTokens).constructor
            ...
            console.log("COLOR VISUAL >>>>>", getConstructorArguments(filteredTokens).constructor)
            let colorVisualArguments = getConstructorArguments(filteredTokens).constructor
            let colorVisual = new ColorVisualData()
            if (colorVisualArguments.length == 1) {
                colorVisual.color = getColor(getKotlinTokens(colorVisualArguments[0].value))
    ...
    function getFont(tokens: Token[]) {
        ...
        if (filteredTokens.length == 0) return null;
        console.log("FONT >>>>>", filteredTokens)
        if(filteredTokens[0].value != "Font") {
            return new Components.FontData()
        } else if (filteredTokens[0].value == "Font") {
            const fontAssignments = getConstructorArguments(filteredTokens).constructor
            ...
            console.log("FONT >>>>>", fontAssignments)
            const font = new Components.FontData()
            fontAssignments.forEach((assignment) => {
                if (assignment.property == "size") {
                    font.size.value = getTypedValue(assignment.value, constructor.find((con) => con.property == "size"))
                } else if (assignment.property == "family") {
                    font.family.value = getTypedValue(assignment.value, constructor.find((con) => con.property == "family"))
                    ...
                    font.fontWeight.value = getTypedValue(assignment.value, constructor.find((con) => con.property == "fontWeight"))
                } else if (assignment.property == "fontStyle") {
                    font.fontStyle.value = getTypedValue(assignment.value, constructor.find((con) => con.property == "fontStyle"))
                }
            })
    ...
    function getColor(tokens: Token[]) {
        ...
        if (filteredTokens[0].value == "Color") {
            const colorAssignments = getConstructorArguments(filteredTokens).constructor
            ...
            switch (colorAssignments.length) {
                ...
                case 3:
                    return convertRGBToHex(`rgb(${colorAssignments[0].value},${colorAssignments[1].value},${colorAssignments[2].value})`)
                case 4:
                    return convertRGBAToHex(`rgba(${colorAssignments[0].value},${colorAssignments[1].value},${colorAssignments[2].value},${colorAssignments[3].value})`)
                default:
                    return "#ffffff"
            }
        } else {
